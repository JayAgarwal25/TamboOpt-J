"""Reconstruction networks for v6: flat MLP and DeepSets (permutation-invariant).

Both classes share the same normalization contract and checkpoint format.
Use build_recon_from_ckpt to load either transparently:

    from modules_v6.reconstruction import build_recon_from_ckpt
    recon = build_recon_from_ckpt(ckpt, N_DETECTORS, DEVICE)

Input convention: both models accept (B, n_det, input_features) in raw units.
Reconstruction also accepts the legacy flat (B, n_det*input_features) shape.
"""
...
class Reconstruction(nn.Module):
    ...
    Accepts (B, n_det, input_features) or legacy flat (B, n_det*input_features).
    Output: [dir_x, dir_y, dir_z, log_e_norm]
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Args: x of shape (B, n_det, input_features) or (B, n_det*input_features).
