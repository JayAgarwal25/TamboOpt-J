...
# the same signatures so this file stays diffable against
# 04_optimize_lbfgs_ensemble.py, which is the point of it being a copy.
...
def _penalty_args(mountain):
    """(mesh_en on DEVICE, r0) for activation_of_xy, built once per mountain.
...
def adam_warm_start(scheme: str,
    ...
    for epoch in range(N_ADAM_EPOCHS):
        ...
        U, r, parts = activation_of_xy(x_det, y_det, primary_batch, fnn,
                                       mode=OBJECTIVE, mesh_en=_mesh,
                                       penalty_w=OFFMESH_PENALTY, penalty_r0=_r0)
        loss = -U
        ...
        log.append(dict(
            ...
            u_particles=float(parts["u_particles"].item()),
            u_detectors=float(parts["u_detectors"].item()),
            u_distinct=float(parts["u_distinct"].item()),
...
def score_on_set(x: torch.Tensor, y: torch.Tensor, scoring_set: torch.Tensor,
    ...
    for s in range(0, scoring_set.shape[0], step):
        ...
        U, _, _ = activation_of_xy(x, y, batch, fnn, mode=OBJECTIVE,
...
def lbfgs_refine(init_x: torch.Tensor,
    ...
    def closure():
        ...
        U, r, parts = activation_of_xy(x_det, y_det, primary_fixed, fnn,
                                       mode=OBJECTIVE, mesh_en=mesh_en,
    ...
    with torch.no_grad():
        ...
        U_proj, _, _ = activation_of_xy(
            x_proj.to(DEVICE), y_proj.to(DEVICE), primary_fixed, fnn,
            mode=OBJECTIVE, mesh_en=mesh_en, penalty_w=OFFMESH_PENALTY,
            penalty_r0=pen_r0,
        )
...
def _run_one_scheme(scheme: str,
    ...
    with open(os.path.join(opt_dir, "optimize_log.json"), "w") as f:
        json.dump({
            ...
            "config": dict(
                ...
                objective=OBJECTIVE, particle_scale=PARTICLE_SCALE,
                distinct_scale=DISTINCT_SCALE, sigma_spatial=SIGMA_SPATIAL,
    ...
    _plt.plot_components_lbfgs(
        all_adam_logs, lbfgs_logs,
        os.path.join(opt_dir, "utility_components.png"),
        # Both activation measures on every run, whichever one U is. They are not
        # summands of U (unlike the composite's θ/φ/E) — the un-maximized one is
        # there to show what the chosen objective cost.
        parts_spec=[(f"particles /{PARTICLE_SCALE:g}", "u_particles", 1.0, "C0"),
                    ("detectors (soft)", "u_detectors", 1.0, "C1"),
                    (f"distinct /{DISTINCT_SCALE:g}", "u_distinct", 1.0, "C2")],
        title=f"per-chain activation decomposition (objective={OBJECTIVE}; "
              f"Adam solid, L-BFGS dashed)")
...
def main():
    ...
    ap.add_argument("--objective",
                    choices=("particles", "detectors", "distinct"),
                    default=OBJECTIVE,
                    help="what to maximize. 'particles' = total flux through the "
                         "detector positions; the kernel double-counts particles "
                         "seen by overlapping detectors, so this one is maximized "
    ...
    ap.add_argument("--offmesh-penalty", type=float, default=OFFMESH_PENALTY,
                    help="weight (in units of the objective) of the differentiable "
                         "off-mesh penalty subtracted inside activation_of_xy. "
                         "0 disables "
    ...
    print("=" * 72)
    print("v6/04_optimize_lbfgs_activation.py — maximize collection, not recon")
    print("=" * 72)
    print(f"device       : {DEVICE}")
    print(f"objective    : {OBJECTIVE}  " + {
        "particles": f"(summed flux / {PARTICLE_SCALE:g}; double-counts overlap)",
        "distinct":  f"(overlap-corrected flux / {DISTINCT_SCALE:g}, "
                     f"sigma={SIGMA_SPATIAL:g} m)",
        "detectors": f"(soft count, tau={TAU_LAYOUT:g}, thr={LAYOUT_THRESHOLD:g})",
    }[OBJECTIVE])
